...
const sellerVideoUrlInput = document.getElementById("sellerVideoUrl");
const saveSellerProfileBtn = document.getElementById("saveSellerProfileBtn");

const previewSellerName = document.getElementById("previewSellerName");
const previewSellerId = document.getElementById("previewSellerId");
const previewSellerContact = document.getElementById("previewSellerContact");
const previewSellerVideoText = document.getElementById("previewSellerVideoText");
const previewSellerVideoLink = document.getElementById("previewSellerVideoLink");
const previewSellerWhatsappBtn = document.getElementById("previewSellerWhatsappBtn");
const previewSellerWhatsappHint = document.getElementById("previewSellerWhatsappHint");
...
const buyerSellerVideoText = document.getElementById("buyerSellerVideoText");
const buyerSellerVideoLink = document.getElementById("buyerSellerVideoLink");
const buyerWhatsappBtn = document.getElementById("buyerWhatsappBtn");
const buyerWhatsappHint = document.getElementById("buyerWhatsappHint");

const checkoutSellerContactText = document.getElementById("checkoutSellerContactText");
const checkoutWhatsappBtn = document.getElementById("checkoutWhatsappBtn");
const checkoutWhatsappHint = document.getElementById("checkoutWhatsappHint");

function normalizeSellerText(value) {
  return typeof value === "string" ? value.trim() : "";
}

function getDefaultSellerProfile() {
  return {
    name: "",
    id: "",
    contact: "",
...
function getStoredSellerProfile() {
  var fallback = getDefaultSellerProfile();
  var saved = localStorage.getItem("sellerProfile");
  if (!saved) return fallback;

  try {
    var seller = JSON.parse(saved) || {};
    return {
      name: normalizeSellerText(seller.name),
      id: normalizeSellerText(seller.id),
      contact: normalizeSellerText(seller.contact),
      videoUrl: normalizeSellerText(seller.videoUrl || seller.video || seller.shortVideoUrl)
    };
  } catch (error) {
    console.warn("Failed to parse sellerProfile from localStorage.", error);
    return fallback;
  }
}

function getSafeExternalUrl(url) {
  var cleanUrl = normalizeSellerText(url);
  if (!cleanUrl) return "";
...
function getVideoLinkLabel(url) {
  if (!url) return "Open short video";
  if (/youtube\.com|youtu\.be/i.test(url)) return "Watch on YouTube";
  if (/\.(mp4|mov|webm|ogg)(\?|#|$)/i.test(url)) return "Open short video";
  return "Open video link";
}

function getWhatsappNumber(contact) {
  var digits = normalizeSellerText(contact).replace(/\D/g, "");
  if (!digits) return "";
...
function getWhatsappLink(contact) {
  var whatsappNumber = getWhatsappNumber(contact);
  if (!whatsappNumber) return "";

  return "https://wa.me/" + whatsappNumber + "?text=" + encodeURIComponent("Hello KING Store, I want to ask about the menu.");
}

function updateOptionalLink(linkEl, textEl, href, label, fallbackText) {
  if (linkEl) {
    if (href) {
      linkEl.href = href;
  ...
  if (textEl) {
    textEl.textContent = href ? "" : fallbackText;
    textEl.classList.toggle("hidden", !!href);
  }
}

function updateWhatsappButton(buttonEl, hintEl, contact) {
  var whatsappLink = getWhatsappLink(contact);

  if (buttonEl) {
    if (whatsappLink) {
      buttonEl.href = whatsappLink;
  ...
  if (hintEl) hintEl.classList.toggle("hidden", !!whatsappLink);
}

function populateSellerProfileForm(profile) {
  if (sellerNameInput) sellerNameInput.value = profile.name || "";
  if (sellerIdInput) sellerIdInput.value = profile.id || "";
  if (sellerContactInput) sellerContactInput.value = profile.contact || "";
  if (sellerVideoUrlInput) sellerVideoUrlInput.value = profile.videoUrl || "";
}

function renderSellerProfilePreview(profile) {
  var videoHref = getSafeExternalUrl(profile.videoUrl);

  if (previewSellerName) previewSellerName.textContent = profile.name || "-";
  if (previewSellerId) previewSellerId.textContent = profile.id || "-";
  if (previewSellerContact) previewSellerContact.textContent = profile.contact || "-";

  updateOptionalLink(
    previewSellerVideoLink,
    previewSellerVideoText,
    videoHref,
    getVideoLinkLabel(videoHref),
    profile.videoUrl || "-"
  );

  updateWhatsappButton(previewSellerWhatsappBtn, previewSellerWhatsappHint, profile.contact);
}

function renderBuyerSellerInfo(profile) {
  var videoHref = getSafeExternalUrl(profile.videoUrl);

  if (buyerSellerName) buyerSellerName.textContent = profile.name || "KING Store";
  if (buyerSellerId) buyerSellerId.textContent = profile.id || "-";
  if (buyerSellerContact) buyerSellerContact.textContent = profile.contact || "-";
  if (checkoutSellerContactText) checkoutSellerContactText.textContent = profile.contact || "-";

  updateOptionalLink(
    buyerSellerVideoLink,
    buyerSellerVideoText,
    videoHref,
    getVideoLinkLabel(videoHref),
    profile.videoUrl || "-"
  );

  updateWhatsappButton(buyerWhatsappBtn, buyerWhatsappHint, profile.contact);
  updateWhatsappButton(checkoutWhatsappBtn, checkoutWhatsappHint, profile.contact);
}

function loadSellerProfile() {
  const seller = getStoredSellerProfile();
  populateSellerProfileForm(seller);
  renderSellerProfilePreview(seller);
  renderBuyerSellerInfo(seller);
}

function saveSellerProfile() {
  const seller = {
    name: normalizeSellerText(sellerNameInput && sellerNameInput.value),
    id: normalizeSellerText(sellerIdInput && sellerIdInput.value),
    contact: normalizeSellerText(sellerContactInput && sellerContactInput.value),
    videoUrl: normalizeSellerText(sellerVideoUrlInput && sellerVideoUrlInput.value)
  };
...
if (saveSellerProfileBtn) {
  saveSellerProfileBtn.addEventListener("click", saveSellerProfile);
}
