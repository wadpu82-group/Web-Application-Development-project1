...
var welcomeLoginBtn = document.getElementById("welcome-login");
if(welcomeLoginBtn){
  welcomeLoginBtn.addEventListener("click", function(){
    if(!isUserRegistered()){
      showToast("Please register first to create an account.", "error");
...
var welcomeRegisterBtn = document.getElementById("welcome-register");
if(welcomeRegisterBtn){
  welcomeRegisterBtn.addEventListener("click", function(){ openRegistrationPage(); });
}

var registerBackBtn = document.getElementById("register-back-welcome");
if(registerBackBtn){
  registerBackBtn.addEventListener("click", function(){ openWelcomePage(); });
}

var loginBackBtn = document.getElementById("login-back-welcome");
if(loginBackBtn){
  loginBackBtn.addEventListener("click", function(){ openWelcomePage(); });
}

var userGreetingEl = document.createElement("button");
userGreetingEl.id = "userGreeting";
userGreetingEl.type = "button";
userGreetingEl.className = "header-profile-btn hidden ml-3";
userGreetingEl.innerHTML =
  '<span id="header-profile-avatar" class="header-profile-avatar">U</span>' +
  '<span id="userGreetingText" class="hidden md:inline">User</span>';
var headerActions = document.querySelector(".header .max-w-6xl");
if(headerActions){
  headerActions.appendChild(userGreetingEl);
  userGreetingEl.addEventListener("click", openProfile);
}
...
function updateUserGreeting(user){
  ...
  var avatar = document.getElementById("header-profile-avatar");
  if(!greeting || !greetingText) return;
  if(user && user.name){
    greetingText.textContent = user.name.split(" ")[0];
    if(avatar) avatar.textContent = getUserInitials(user.name);
 ...
 * Free OpenStreetMap geocoding. Indonesia-biased first (countrycodes=id) so
 * "Cikarang", "Monas", etc. resolve to local results. Falls back to a global
 * search when the Indonesia-only query returns nothing, so users can still
 * pin overseas addresses (e.g. "Beverly Hills"). Reuses the existing
 * setSelectedCheckoutLocation() so the map marker, coordinates state, and
 * checkout payload all stay in sync.
 */
var locationSearchDebounceTimer = null;
var locationSearchAbortController = null;

var ID_LOCATION_KEYWORDS = [
  "cikarang","bekasi","jakarta","jakut","jaksel","jaktim","jakbar","jakpus",
  "tangerang","tangsel","depok","bogor","bandung","cimahi","cirebon","sukabumi",
  "garut","tasikmalaya","semarang","yogyakarta","yogya","jogja","solo","surakarta",
  "surabaya","sidoarjo","malang","kediri","madiun","probolinggo","banyuwangi",
  "denpasar","bali","ubud","lombok","mataram","medan","palembang","padang",
  "pekanbaru","makassar","manado","balikpapan","samarinda","banjarmasin",
  "pontianak","jayapura","ambon","kupang","aceh","banten","jawa","sumatra","kalimantan","sulawesi","papua",
  "indonesia","gading","serpong","karawang","purwakarta","subang"
];

function detectIdLocationKeyword(query){
  var lower = (query || "").toLowerCase();
  for(var i = 0; i < ID_LOCATION_KEYWORDS.length; i++){
    var kw = ID_LOCATION_KEYWORDS[i];
    if(new RegExp("\\b" + kw + "\\b").test(lower)) return kw;
  }
  return null;
}

function buildQueryVariants(query, idKeyword){
  var variants = [query];
  var stripped = query.replace(/\b(new|old|the|de|la|le)\b/gi, "").replace(/\s+/g, " ").trim();
  if(stripped && stripped !== query) variants.push(stripped);
  if(idKeyword){
    variants.push(idKeyword);
  } else {
    var firstTwoWords = query.split(/\s+/).slice(0, 2).join(" ").trim();
    if(firstTwoWords && variants.indexOf(firstTwoWords) === -1) variants.push(firstTwoWords);
  }
  return variants;
}

function isIndonesiaResult(item){
  if(!item) return false;
  var name = (item.display_name || "").toLowerCase();
  return name.indexOf("indonesia") !== -1;
}
...
async function geocodeWithNominatim(query, limit){
  ...
  var signal = locationSearchAbortController.signal;

  function buildUrl(q, idOnly){
    var url = "https://nominatim.openstreetmap.org/search" +
      "?format=json" +
      "&q=" + encodeURIComponent(q) +
      "&limit=" + (limit || 5) +
      "&addressdetails=1";
    if(idOnly) url += "&countrycodes=id";
    return url;
  }
  ...
  try {
    var idKeyword = detectIdLocationKeyword(trimmed);
    var variants = buildQueryVariants(trimmed, idKeyword);

    for(var i = 0; i < variants.length; i++){
      var v = variants[i];
      var localResults = await fetchJson(buildUrl(v, true));
    ...
    for(var j = 0; j < variants.length; j++){
      var globalResults = await fetchJson(buildUrl(variants[j], false));
...
function renderLocationSuggestions(results){
  ...
  for(var i = 0; i < results.length; i++){
    var r = results[i];
    var safeName = escapeHtml(r.display_name || "");
    html +=
      '<li class="location-suggestion-item" data-lat="' + r.lat + '" data-lng="' + r.lon + '" data-name="' + safeName + '">' +
        '<span class="location-suggestion-icon">📍</span>' +
        '<span class="location-suggestion-text">' + safeName + '</span>' +
      '</li>';
  }
  list.innerHTML = html;
...
function applyGeocodingResult(result){
  if(!result) return;
  var lat = parseFloat(result.lat);
  var lng = parseFloat(result.lon !== undefined ? result.lon : result.lng);
  if(isNaN(lat) || isNaN(lng) || !hasValidCoordinates(lat, lng)){
    setLocationSearchStatus("Invalid coordinates from result.", "error");
    return;
  }

  setSelectedCheckoutLocation(lat, lng);

  // Auto-fill address if user hasn't typed one yet (convenience).
  var addressInput = document.getElementById("buyerAddress");
  if(addressInput && !addressInput.value.trim() && result.display_name){
    addressInput.value = result.display_name;
    clearCheckoutError();
  }

  renderLocationSuggestions([]);
  setLocationSearchStatus("✅ Location pinned: " + (result.display_name || "selected coordinates"), "success");
}
...
async function handleLocationSearchInput(event){
  ...
  locationSearchDebounceTimer = setTimeout(async function(){
    setLocationSearchStatus("Searching...", "loading");
    var results = await geocodeWithNominatim(value, 5);
    if(results === null) return; // request aborted by newer keystroke
    if(results.length === 0){
      var idKw = detectIdLocationKeyword(value);
      var msg = idKw
        ? "POI ini belum terdaftar di OpenStreetMap. Coba nama jalan/area (mis. 'Jl. Movie Land " + idKw.charAt(0).toUpperCase() + idKw.slice(1) + "') atau klik peta di bawah untuk pin manual."
        : "Not found. Try a street/area name or click the map below to pin manually.";
      setLocationSearchStatus(msg, "error");
      renderLocationSuggestions([]);
    } else {
      setLocationSearchStatus("");
...
async function handleLocationSearchSubmit(){
  ...
  if(!value){
    setLocationSearchStatus("Please type a location first.", "error");
    return;
  }
  setLocationSearchStatus("Searching...", "loading");
  var results = await geocodeWithNominatim(value, 1);
  if(results === null) return;
  if(results.length === 0){
    var idKw = detectIdLocationKeyword(value);
    var msg = idKw
      ? "POI '" + value + "' belum terdaftar di OpenStreetMap (database berbeda dari Google Maps). Coba cari nama jalan/area di " + idKw.charAt(0).toUpperCase() + idKw.slice(1) + ", atau klik peta di bawah untuk pin manual."
      : "Not found in OpenStreetMap. Try a street/area name, or click the map below to pin manually.";
    setLocationSearchStatus(msg, "error");
    renderLocationSuggestions([]);
    return;
  }
  applyGeocodingResult(results[0]);
}

(function attachLocationSearchHandlers(){
  var input = document.getElementById("location-search-input");
  var btn = document.getElementById("location-search-btn");
  var list = document.getElementById("location-suggestions");

  if(input){
    input.addEventListener("input", handleLocationSearchInput);
  ...
  if(btn) btn.addEventListener("click", handleLocationSearchSubmit);
  if(list){
    list.addEventListener("click", function(e){
      var item = e.target.closest(".location-suggestion-item");
      if(!item) return;
      applyGeocodingResult({
        lat: item.getAttribute("data-lat"),
        lon: item.getAttribute("data-lng"),
  ...
  document.addEventListener("click", function(e){
    var dropdown = document.getElementById("location-suggestions");
...
function openMyOrdersFromProfile(){
  closeProfile();
  setTimeout(openMyOrders, 200);
}

function calculateUserStats(myOrders){
  var totalSpent = 0;
  var categoryCount = {};
  ...
  for(var i = 0; i < myOrders.length; i++){
    ...
    for(var j = 0; j < items.length; j++){
      var line = items[j] || {};
      var qty = getItemQty(line);
      var cats = Array.isArray(line.cat) ? line.cat : [];
      for(var k = 0; k < cats.length; k++){
        var c = cats[k];
        if(!c) continue;
        categoryCount[c] = (categoryCount[c] || 0) + qty;
      }
    }
  }
...
function getMemberSinceLabel(myOrders){
  if(!myOrders || myOrders.length === 0) return "New Member";
  var oldest = myOrders[myOrders.length - 1];
  if(!oldest || !oldest.date) return "New Member";
  var d = new Date(oldest.date);
...
function renderProfile(){
  ...
  var stats = calculateUserStats(myOrders);

  var statOrders = document.getElementById("profile-stat-orders");
  var statSpent = document.getElementById("profile-stat-spent");
  var statFav = document.getElementById("profile-stat-favorite");

  if(statOrders) statOrders.textContent = stats.totalOrders;
  if(statSpent) statSpent.textContent = formatRupiah(stats.totalSpent);
  if(statFav) statFav.textContent = stats.favoriteCategory;
  if(memberSince) memberSince.textContent = getMemberSinceLabel(myOrders);

  var recent = document.getElementById("profile-recent-orders");
  if(recent){
    if(myOrders.length === 0){
      recent.innerHTML = '<p class="profile-empty">No orders yet. Start ordering from the menu!</p>';
    } else {
      var recentSlice = myOrders.slice(0, 3);
      var html = "";
      for(var i = 0; i < recentSlice.length; i++){
        var order = normalizeOrder(recentSlice[i], i);
        var itemCount = Array.isArray(order.items) ? order.items.length : 0;
        html +=
          '<div class="profile-recent-order-item">' +
            '<div class="profile-recent-order-info">' +
              '<div class="profile-recent-order-title">' + escapeHtml(getOrderDisplayId(order, i)) + '</div>' +
              '<div class="profile-recent-order-meta">' + itemCount + ' item(s) · ' + escapeHtml(order.dateLabel) + '</div>' +
            '</div>' +
            '<div class="profile-recent-order-total">' + formatRupiah(order.total) + '</div>' +
          '</div>';
      }
...
function updateSellerStats(){
  ...
  for(var k = 0; k < orders.length; k++){
    var ord = normalizeOrder(orders[k], k);
    totalRevenue += Number(ord.total) || 0;
    for(var m = 0; m < ord.items.length; m++){
      totalItems += getItemQty(ord.items[m]) || 0;
    }
  }
  var elOrders = document.getElementById("stat-total-orders");
  var elRevenue = document.getElementById("stat-total-revenue");
  var elItems = document.getElementById("stat-items-sold");
  ...
  if(orders.length === 0){
    list.innerHTML =
      '<div class="seller-empty-state">' +
        ...
        '<p class="text-orange-100">New orders from customers will appear here</p>' +
      '</div>';
    return;
  }
  ...
  for(var i = 0; i < orders.length; i++){
    ...
    html +=
      '<div class="order-card">' +
        '<div class="order-card-header">' +
          '<div>' +
            '<span class="order-id-pill">🧾 ' + getOrderDisplayId(order, i) + '</span>' +
            '<div class="order-meta" style="margin-top:0.55rem;"><strong>Buyer:</strong> ' + escapeHtml(order.buyerName) + '</div>' +
            '<div class="order-meta"><strong>Address:</strong> ' + escapeHtml(order.buyerAddress).replace(/\n/g, "<br>") + '</div>' +
            getOrderLocationHtml(order) +
          '</div>' +
          '<span class="order-date-pill">📅 ' + escapeHtml(order.dateLabel) + '</span>' +
        '</div>' +
...
function renderSellerOrders() {
  var list = document.getElementById("seller-orders-list");
  if (!list) return;

  var orders = getAllOrders();
  if (!orders || orders.length === 0) {
    list.innerHTML = '<div class="seller-empty-state">No orders yet. Waiting for customers...</div>';
    return;
  }
  ...
  for (var i = 0; i < orders.length; i++) {
    ...
    var statusClass = isCompleted ? "status-completed" : "status-pending";
    ...
    if (order.items && order.items.length > 0) {
      for (var j = 0; j < order.items.length; j++) {
        var item = order.items[j];
        itemsHtml += '<div class="order-item"><span class="item-name">' + escapeHtml(item.name || "Unknown") + '</span><span class="item-qty">×' + item.quantity + '</span><span class="item-price">Rp ' + formatCurrency(item.price * item.quantity) + '</span></div>';
      }
    }
    
    var mapLink = order.buyerLatitude && order.buyerLongitude ? 'https://maps.google.com/?q=' + order.buyerLatitude + ',' + order.buyerLongitude : '#';
    var whatsappLink = 'https://wa.me/6289518631090?text=Order%20%23' + order.id;
    
    html += '<div class="seller-order-card"><div class="order-header"><span class="order-id">Order #' + order.id + '</span><span class="order-status ' + statusClass + '">' + statusText + '</span></div>' +
      '<div class="order-buyer-info"><div class="buyer-info-row"><span class="buyer-label">👤 Buyer:</span><span class="buyer-value">' + escapeHtml(order.buyerName || "N/A") + '</span></div>' +
      '<div class="buyer-info-row"><span class="buyer-label">📍 Address:</span><span class="buyer-value">' + escapeHtml(order.buyerAddress || "N/A") + '</span></div></div>' +
      '<div class="order-items"><div class="items-title">🍽️ Items:</div>' + itemsHtml + '</div>' +
      '<div class="order-total">💰 Total: Rp ' + formatCurrency(order.total) + '</div>' +
      (order.buyerLatitude && order.buyerLongitude ? '<div class="order-location"><div class="location-label">📍 Coordinates:</div><div class="location-coords"><span class="coord-badge">Lat: ' + order.buyerLatitude.toFixed(4) + '</span><span class="coord-badge">Lng: ' + order.buyerLongitude.toFixed(4) + '</span></div></div>' : '') +
      '<div class="order-actions">' +
      (!isCompleted ? '<button onclick="completeOrder(' + order.id + ')" class="action-btn btn-complete">✅ Complete</button>' : '') +
      '<a href="' + mapLink + '" target="_blank" class="action-btn btn-map">🗺️ Map</a>' +
      '<a href="' + whatsappLink + '" target="_blank" class="action-btn btn-whatsapp">💬 WhatsApp</a>' +
      '</div></div>';
  }
  list.innerHTML = html;
}
...
setTimeout(function(){ renderSellerOrders(); }, 500);
