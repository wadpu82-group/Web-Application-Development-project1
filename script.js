...
function applyStaticSellerInfo() {
  if (sellerStaticName) sellerStaticName.textContent = STATIC_SELLER.name;
  if (sellerStaticId) sellerStaticId.textContent = STATIC_SELLER.id;
  if (sellerStaticContact) sellerStaticContact.textContent = STATIC_SELLER.contact;
  if (buyerSellerName) buyerSellerName.textContent = STATIC_SELLER.name;
  if (buyerSellerId) buyerSellerId.textContent = STATIC_SELLER.id;
  if (buyerSellerContact) buyerSellerContact.textContent = STATIC_SELLER.contact;
  if (checkoutSellerContactText) checkoutSellerContactText.textContent = STATIC_SELLER.contact;

  if (sellerStaticWhatsappBtn) sellerStaticWhatsappBtn.href = STATIC_SELLER.whatsappLink;
  if (buyerWhatsappBtn) buyerWhatsappBtn.href = STATIC_SELLER.whatsappLink;
  if (checkoutWhatsappBtn) checkoutWhatsappBtn.href = STATIC_SELLER.whatsappLink;
}

applyStaticSellerInfo();
