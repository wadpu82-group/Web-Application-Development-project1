...
// Keep blocked menu/category removals in one place so fallback and Supabase data share the same runtime rules.
const RUNTIME_HIDDEN_MENU_NAMES = Object.freeze([
  "tiramisu",
  "strawberry mochi",
  "dubai chocolate mochi pistachio",
  "cheesecake brownies"
]);
...
function smartChatSearch(userText) {
  ...
  if (isColdOfferRequest(text)) {
    ...
    if (coldOffers.length > 0) {
      appendMsg("Kamu cari minuman dingin ya? Ini beberapa pilihan segar untukmu:", "bot", coldOffers);
      return;
    }
  }
  ...
  if (detectedItems.length > 0) {
    ...
    appendMsg("Saya mendeteksi menu ini dari chat kamu:", "bot", itemsToShow);
    ...
    if (hasOrderIntent(userText)) {
      ...
      appendMsg("✅ Pesanan ditambahkan ke daftar chatbot: " + addedNames.join(", ") + ".", "bot");
      return;
    }
  ...
  if (intents.length > 0) {
    ...
    if (filtered.length > 0) {
      showChatbotMenuSuggestions(filtered, "Berikut menu yang cocok dengan permintaan kamu:");
      return;
    }
  }
  ...
  if (!ok) {
    ...
    if (popular.length > 0) {
      appendMsg("Saya tidak yakin, tapi ini beberapa menu populer yang bisa kamu pesan:", "bot", popular);
      return;
    }
    appendMsg("Maaf, saya belum menemukan menu yang cocok. Coba kata kunci lain atau pilih kategori menu.", "bot");
  }
}
...
function isUserRegistered(){
  try {
    var stored = localStorage.getItem("kingStoreUser");
    return !!stored;
  } catch (error) {
    return false;
  }
...
function getRegisteredUser(){
  try {
    return JSON.parse(localStorage.getItem("kingStoreUser") || "{}");
  } catch (error) {
    return {};
  }
}
...
function logoutUser(){
  ...
  openLoginPage();
  showToast("Kamu telah logout.", "success");
}
...
function validateRegistrationForm(){
  ...
  if(!nameInput || !nameInput.value.trim()){
    ...
    if(error){ error.textContent = "Nama lengkap wajib diisi."; error.classList.remove("hidden"); }
    isValid = false;
  }
  ...
  if(!phoneValue || !/^\d+$/.test(phoneValue)){
    ...
    if(error){ error.textContent = "Nomor telepon wajib diisi dan hanya boleh angka."; error.classList.remove("hidden"); }
    isValid = false;
  }

  if(!addressInput || !addressInput.value.trim()){
    var error = document.getElementById("error-address");
    if(error){ error.textContent = "Alamat lengkap wajib diisi."; error.classList.remove("hidden"); }
    isValid = false;
  }
  ...
  if(!emailValue || !validateEmail(emailValue)){
    ...
    if(error){ error.textContent = "Mohon masukkan alamat email yang valid."; error.classList.remove("hidden"); }
    isValid = false;
  }
  ...
  if(!passwordValue || passwordValue.length < 6){
    ...
    if(error){ error.textContent = "Password wajib diisi minimal 6 karakter."; error.classList.remove("hidden"); }
    isValid = false;
  }
...
function storeRegisteredUser(user){
  try {
    localStorage.setItem("kingStoreUser", JSON.stringify(user));
  } catch (error) {
    console.warn("Unable to store user data:", error);
  }
}
...
function validateLoginForm(){
  ...
  if(!emailValue || !validateEmail(emailValue)){
    ...
    if(error){ error.textContent = "Email wajib diisi dan harus valid."; error.classList.remove("hidden"); }
    isValid = false;
  }
  ...
  if(!passwordValue){
    ...
    if(error){ error.textContent = "Password wajib diisi."; error.classList.remove("hidden"); }
    isValid = false;
  }
...
function handleLoginSubmit(event){
  ...
  if(!registeredUser || registeredUser.email !== emailValue || registeredUser.password !== passwordValue){
    var error = document.getElementById("error-login-password");
    if(error){ error.textContent = "Email atau password tidak cocok."; error.classList.remove("hidden"); }
    return;
  }
  ...
  updateUserGreeting(registeredUser);
  ...
  setTimeout(function(){
    ...
    closeLoginPage();
    closeRegistrationPage();
    showToast("Selamat datang kembali, " + registeredUser.name + "!", "success");
  }, 500);
}
...
function renderMenu(items){
  ...
  for(var i = 0; i < items.length; i++){
    ...
    if(item.price >= 50000 && !isRuntimeHiddenCategory("premium")) badges += '<span class="absolute top-2 left-2 premium-badge text-white text-xs px-2 py-1 rounded-full">💎 Premium</span>';
...
function saveCart(){
  // Sanitize locally saved selections so hidden items cannot resurface from old carts.
...
function appendTypingIndicator() {
  ...
  wrapper.innerHTML = '<div class="bg-orange-700 text-white p-3 message-bubble text-sm shadow-sm font-bold">⏳ Sedang berpikir...</div>';
...
async function handleChatWithGroq(userText) {
  ...
  try {
    ...
    appendMsg(result.reply || "Maaf, saya belum punya jawaban.", "bot", items && items.length ? items : undefined);
  } catch(err) {
    removeTypingIndicator();
    console.warn("Groq AI gagal, fallback ke smart search:", err);
    appendMsg("⚠️ AI sedang sibuk, saya bantu cari manual ya...", "bot");
