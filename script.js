...
async function geocodeWithNominatim(query, limit){
  ...
  try {
    var variants = buildQueryVariants(trimmed);
    for(var i = 0; i < variants.length; i++){
      var v = variants[i];
      var localResults = await fetchJson(buildUrl(v, true));
      if(localResults.length > 0) return localResults;
      var globalResults = await fetchJson(buildUrl(v, false));
...
async function handleLocationSearchInput(event){
  ...
  locationSearchDebounceTimer = setTimeout(async function(){
    ...
    if(results.length === 0){
      setLocationSearchStatus("Not found. Try a street/area name (e.g. 'Jl. Movie Land Cikarang') or click the map below to pin manually.", "error");
      renderLocationSuggestions([]);
    } else {
      setLocationSearchStatus("");
...
async function handleLocationSearchSubmit(){
  ...
  if(results.length === 0){
    setLocationSearchStatus("Not found in OpenStreetMap (different from Google Maps). Try a street/area name like 'Jl. Movie Land Cikarang Utara', or click the map below to pin manually.", "error");
    renderLocationSuggestions([]);
    return;
  }
